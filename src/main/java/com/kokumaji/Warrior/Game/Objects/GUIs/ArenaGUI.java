...
import com.kokumaji.Warrior.Utils.WarriorItem;
...
public class ArenaGUI extends GUI {
    ...
    public void BuildGUI(Player player) {
        ...
        ItemStack placeholder = new WarriorItem(Material.BLACK_STAINED_GLASS, 1, " ").build();
        for(int i = 0; i < 9; i++) {
        ...
        for(Arena a : ArenaManager.GetArenas().values()) {
            if(j > 36) break;
            ItemStack arenaItem = CustomItem.Create(Material.FILLED_MAP, "§3§l§o" + a.GetName(), 1, true, "§7Teleport to Arena §b" + a.GetName());
    ...
    protected void onSlotClick(InventoryClickEvent e) {
        ...
        if(IsValidSlot(slot)) {
            ...
            if(slot > 8 && slot < 37) {
                ...
                WarriorUser u = UserManager.GetPlayer(e.getWhoClicked().getUniqueId());
